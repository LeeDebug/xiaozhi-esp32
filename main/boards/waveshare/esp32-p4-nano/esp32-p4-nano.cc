...
private:
    ...
    static esp_err_t EnableDsiPhyPower() {
        ...
        ESP_RETURN_ON_ERROR(esp_ldo_acquire_channel(&ldo_cfg, &phy_pwr_chan), TAG,
                            "enable MIPI DSI PHY power failed");
    ...
    void InitializeLCD() {
        const uint8_t write_cmds[][2] = {{0x95, 0x11}, {0x95, 0x17}, {0x96, 0x00}, {0x96, 0xFF}};
        ...
        if (i2c_master_bus_add_device(codec_i2c_bus_, &i2c_dev_conf, &dev_handle) == ESP_OK) {
            for (const auto &cmd : write_cmds) {
                ESP_ERROR_CHECK(i2c_master_transmit(dev_handle, cmd, sizeof(cmd), 50));
            }
        ...
        const jd9365_vendor_config_t vendor_config = {
            ...
            .mipi_config = {.dsi_bus = mipi_dsi_bus, .dpi_config = &dpi_config, .lane_num = 2},
        };
        const esp_lcd_panel_dev_config_t lcd_dev_config = {
            .reset_gpio_num = PIN_NUM_LCD_RST,
            .rgb_ele_order = LCD_RGB_ELEMENT_ORDER_RGB,
            .bits_per_pixel = 16,
            .vendor_config = const_cast<jd9365_vendor_config_t *>(&vendor_config),
        };
